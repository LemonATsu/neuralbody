...
class Dataset(data.Dataset):
    def __init__(self, data_root, subject, ann_file, split, mul=1.05):
        ...
        idxs = np.load(os.path.join(data_root, f'{subject}_selected.npy'))
        self.ims = idxs#np.array(annots['ims'])[idxs]
        self.cam_inds = idxs
    ...
    def _init_dataset(self):
        if self.subject.startswith('James'):
            self._dataset = h5py.File(os.path.join(self.data_root, 'James_NB_h5py.h5'))
        elif self.subject.startswith('Archer'):
            self._dataset = h5py.File(os.path.join(self.data_root, 'Archer_NB_h5py.h5'))
        else:
            raise NotImplementedError(f'Subject {self.subject} not implemented.')
        self.body_data = {'Rh': self._dataset['Rh'][:],
                          'Th': self._dataset['Th'][:],
                          'vertices': self._dataset['vertices'][:]}
        self.bkgds = self._dataset['bkgds'][:]
        self.bkgd_idxs = self._dataset['bkgd_idxs'][:].astype(np.int64)
    ...
    def get_mask(self, index):
        ...
        msk = self._dataset['masks'][self.ims[index]]
    ...
    def prepare_input(self, index):
        ...
        xyz = self._dataset['vertices'][self.ims[index]]
        ...
        Th = self.body_data['Th'][self.ims[index]].astype(np.float32) #* self.mul
        xyz = np.dot(xyz - Th, R)
    ...
    def __getitem__(self, index):
        ...
        img = self._dataset['imgs'][self.ims[index], ..., :3] / 255.
